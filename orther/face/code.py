...
class FaceDetectorApp:
    ...
    def setup_styles(self):
        ...
        style.configure("Control.TButton", padding=6)
    ...
    def create_widgets(self):
        ...
        title_label = ttk.Label(title_frame, text="人脸识别", style="Title.TLabel")
        ...
        # 选择图片按钮
        self.select_btn = ttk.Button(button_frame, text="选择图片", command=self.select_image, style="Control.TButton")
        self.select_btn.pack(side=tk.LEFT, padx=5)
        
        # 检测人脸按钮
        self.detect_btn = ttk.Button(button_frame, text="检测人脸", command=self.detect_faces, state=tk.DISABLED, style="Control.TButton")
        self.detect_btn.pack(side=tk.LEFT, padx=5)
        
        # 保存结果按钮
        self.save_btn = ttk.Button(button_frame, text="保存结果", command=self.save_image, state=tk.DISABLED, style="Control.TButton")
        self.save_btn.pack(side=tk.LEFT, padx=5)
        
        # 参数设置框架
        param_control_frame = ttk.Frame(control_frame)
        param_control_frame.pack(side=tk.TOP, fill=tk.X)
        
        # 参数设置
        param_frame = ttk.LabelFrame(param_control_frame, text="检测参数", padding=10)
