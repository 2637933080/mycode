...
void ReadFile(SqList& L, string infoFilename, string detailDir) {
    // 从文件中读取股票信息，将其按顺序存入L.elem指向的数组中，数组下标从0开始存储
    ifstream infile;
...
double GetASL(SqList& L) {//基于平衡二叉判定树的内部路径长度计算ASL
    if (L.length == 0) return 0;
    int n = L.length;
    // 计算树的高度s = ceil(log2(n+1))
    int s = ceil(log2(n + 1));
    // 前s-1层的节点数
    int full_nodes = pow(2, s - 1) - 1;
    // 第s层的节点数
    int last_level_nodes = n - full_nodes;
